...
| ME_MMS_030_010 a| . Use PCIe discovery to locate the RISC-V PCIe test card.
                    . Map the BAR 0 of the test card as IO memory using PBMT
                      attribute.
                    . Write data values to the scratchpad buffer in BAR 0 using
                      and read it back.
...
| MF_MMS_060_010 a| This test requires the use of two RISC-V PCIe test cards
                    connected to two root ports in the system. This test is
                    optional if peer-to-peer DMA is not supported by the system.
                    One test card - card-0 - is used as an initiator and the
                    second card - card-1 - is used as a responder.

                    . Disable poisoned TLP egress blocking in the root ports
                      connecting to the card-0 and card-1.
                    . Program the card-0 to read from the `TEST_POISON_REG` in
                      BAR 0 of card-1.
                    . Verify that card-0 receives completion with EP=1.
...
| MF_MMS_070_010 a| . Use PCIe discovery to locate the RISC-V PCIe test card and
                    ...
                    . Verify that either a data-corruption exception occurs on
