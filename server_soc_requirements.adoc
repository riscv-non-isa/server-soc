...
A RISC-V SoC includes a RISC-V application processor and may include one or
more service processors. These service processors may provide services such as
...
2+| _Ssccfg, Sscind, and Ssctr are under construction._                       +
    ...
    _Many of these mandated extensions are optional in the RVA23 ISA profile.
     ...
     moved downstream into a platform specification (as a chapter) or as a new
     ISA profile specification._
...
| IIC_020  | External interrupts MUST be signaled to a hart as message-signaled
             interrupts (MSI).
2+| _Since MSI are a memory write, it allows for a simplified implementation of
     producer-consumer ordering rules. For example, interrupts following a write
     from a device must only be observed after the writes have been observed,
     and the read completion generated by a device following an interrupt must
     only be observed after the interrupt has been observed._                  +
                                                                               +
    _Message Signaled interrupts (MSI) are the preferred interrupt signaling
     mechanism in PCIe. Supporting MSI for harts allows low-latency interrupt
     signaling._
...
2+| _Supporting 5 VS-mode interrupt files for a hart allows context switching
     ...
     beneficial when devices are directly assigned to virtul machines
...
| IIC_070 a| The memory regions for IMSIC interrupt files MUST have the
             ...
             * Support 4-byte aligned reads and writes.
...
| IOM_170  | IOMMU that supports PASID capability MUST support 20-bit PASID
             width and MAY support 8 and 17 bit PASID widths.
...
| IOM_190  | IOMMU that supports an HPM, MUST support the cycles counter and at
...
| ADR_040  | The host bridge MAY support access to peer device memory. If peer
             ...
             then such accesses MUST be responded to with a UR/CA response. The
...
| ADR_050  | When a posted or non-posted-with-data request is allowed to access
             peer device memory, then the poisoned data (EP=1) MUST be forwarded
             as poisoned data unless such forwarding is disallowed (e.g.,
             poisoned TLP egress blocking, data poisoning is not supported by
             SoC).

| ADR_060  | Host memory writes caused by posted or non-posted-with-data requests
             with poisoned data (EP=1) MUST mark the data in the host memory as
             poisoned.
...
2+| _A PCIe requester is permitted to set the “No Snoop” in transactions it
    ...
    _Some caches in a SoC may perform clean evictions to memory. In such SoCs,
     ...
     required to flush data that may already be cached._             +
                                                                               +
...
2+|_When an uncorrected data error is detected by a component, it might allow
   ...
   _The technique of data poisoning facilitates delaying the handling of
    ...
    for targeted recovery actions that impact only the affected components.    +
                                                                               +
   ...
   _Data poisoning also empowers the implementation of error containment
    features supported by industry standards like PCIe and CXL.                +
...
Protocol and Data Model (SPDM) for device attestation (See DSP0274
cite:[DS0274]) and secure communication using SPDM encrypted messages (See
DSP0277 cite:[DSP0277]) for in-band and out-of-band communication with the BMC.
SPDM authentication protocols support establishing a trust relationship between
the in-band and out-of-band manageability agents in the SoC and the BMC. Use of
SPDM secured messages enables preserving the confidentiality and integrity of
...
Interface (IPMI) for cite:[IPMI20] due to the widespread use of this protocol
...
the datacenter-ready secure control module (DC-SCM) specified by the Open
Compute Project for server management, security, and control features.

Adhering to the industry standard management protocols such as those specified
by DMTF allows server platforms built with RISC-V server SoCs to seamlessly
integrate into the server management frameworks and tools employed by data
...
|===
| ID#     ^| Requirement
| TRC_010 a| The SoC MUST support either the RISC-V E-trace cite:[ETRACE] or the
