...
|===
| ID#            ^| Algorithm
| ME_CTI_010_010 a| Determine the time base frequency from the system hardware
                    table and verify it is equal to 1 GHz.
| ME_CTI_020_010 a| On each RISC-V application processor hart, query the
                    system hardware table to determine the low-power idle states
                    supported by that hart. If a low-power idle state is
                    supported then verify that the timer context is preserved
                    across that low-power idle state.
|===
...
| ME_IIC_010_010 a| For each application processor hart:
                    ...
                    . Determine the ISA string for the hart from the system
                      hardware table.
                    . Parse the ISA string in the ISA node and verify that Ssaia
                      extension is supported.
                    . Determine the interrupt controller connected to the hart
                      from the system hardware table and verify it is an IMSIC.
...
| MF_IIC_030_010 a| . Verify presence of `siselect`, `sireg`, `stopi`, and
                    ...
                    . Map the physical address of the S-mode interrupt register
                      ...
                      IO. The physical address is determined using the system hardware
                      table.
...
| ME_IIC_050_010 a| Querying the system hardware table to verify that the number
                    of supported supervisor mode interrupt identities by the
                    IMSIC is at least 255.
| ME_IIC_060_010 a| Querying the system hardware table to verify that the number
                    of supported guest mode interrupt identities in IMSIC is at
                    least 63.
| ME_IIC_070_010 a| See MF_IIC_030_010.
| ME_IIC_080_010 a| . Parse the system hardware table to determine if an APLIC
                      for supervisor interrupt domain is reported.
                    . If no APLIC is reported then skip the remaining steps.
                    . Locate the APLIC related information in the system hardware
                      table.
                    . Verify that number of interrupt delivery controls
                      is reported as 0 indicating it is used as a wired-to-MSI
                      bridge.
                    . Verify that `domaincfg` supports MSI delivery mode and is
...
| ME_IOM_020_010 a| . Use PCIe discovery to locate all RCiEPs and PCIe RPs.
                    . Locate the system hardware table entries of all IOMMUs
...
| ME_IOM_040_010 a| For each IOMMU that does not govern a PCIe root port:
                    . From the system hardware table entry of that IOMMU,
                      determine the widest device ID.
                    . Verify that the `ddtp` supports a mode that supports the
                      widest device ID.
| ME_IOM_050_010 a| . Parse ISA string reported by the  system hardware tables
                      and determine the page based virtual memory systems
...
| ME_IOM_110_010 a| . Use PCIe discovery to locate all RCiEPs.
                    . For each RCiEP:
                      .. If PCIe ATS capability not supported by the RCiEP
                         then continue.
                      .. Locate the governing IOMMU using system hardware table.
...
| ME_IOM_260_010 a| . Parse the PCIe root complex device binding information from
                      the system hardware table and build a mapping of root
...
|===
| ID#            ^| Algorithm
| MF_ECM_010_010 a| . Parse system hardware table to local all ECAM ranges.
...
| MF_ECM_030_010 a| . Parse system hardware table and obtain ECAM ranges for all
                      heirarchies.
...
|===
| ID#            ^| Algorithm
| ME_MMS_010_010 a| Use system hardware table to locate PCI host bridges and
...
|===
| ID#            ^| Algorithm
| OE_QOS_010_010 a| . Using the system hardware table, determine the ISA string
                      for hart 0.
                    . Parse the ISA string and report in test output log if
                      Ssqosid extension is supported.
                    . Determine if the system hardware table enumerates support
                      for the CBQRI extension. If present report support for the
                      CBQRI extension in test output log.
...
| OE_QOS_040_010 a| If CBQRI extension is supported, as determined by system
                    table, then report `capabilities.QOSID` bit of each
                    IOMMU in the test output log.
| OE_QOS_050_010 a| If CBQRI is not supported then this test is skipped.

                    . Determine caches in the Soc using the system hardware
                      table.
                    . Determine if there is a capacity controller implemented by
                      that cache by looking up the cache ID in system hardware
                      table and report in test output log whether capacity
                      allocation and capacity monitoring are supported by that
                      capacity controller by accessing the controllers
                      capabilities register.
                    . Using the system hardware table, locate all bandwidth
...
| ME_QOS_090_010 a| If CBQRI is supported then determine the RCID and MCID count
                    reported for each capacity and bandwidth controllers in the
                    system hardware table. Verify that each controller supports
...
|===
| ID#            ^| Algorithm
| OF_SPM_010_010 a| . Determine caches in the Soc using the system hardware table
...
| OF_SPM_020_010 a| . Obtain the memory ranges from system hardware table and
