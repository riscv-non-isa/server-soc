...
|===
| ID#            ^| Algorithm
| ME_CTI_010_010 a| On each RISC-V application-processor hart, fix the operating
                    frequency to the lowest supported value. If the
                    implementation does not support operating at a fixed
                    frequency then this test is skipped. Let the fixed operating
                    frequency by F. Poll on the time and cycles CSRs till the
                    time CSR value has advanced by 1 billion. Determine if the
                    cycles CSR has advanced by F.
| ME_CTI_020_010 a| On each RISC-V application-processor hart, set the hart’s
...
| MF_IIC_030_010 a| For each application processor hart:
                    . Determine the ISA string for the hart from the system
                      information table.
                    . Parse the ISA string and verify that Ssaia extension is
                      supported.
                    . Determine the interrupt controller connected to the hart
                      from the system information table and verify it is an
                      IMSIC.
                    . Verify presence of `siselect`, `sireg`, `stopi`, and
                      `stopei` CSRs.
